...
public class TraderSpawnSavedData extends WorldSavedData{
	...
	{
		TraderSpawnSavedData instance = world.getServer().getWorld(DimensionType.OVERWORLD).getSavedData().get(() -> {
			return new TraderSpawnSavedData();
			}, WTImprovements.MOD_ID + ":trader_spawn_data");
		if (instance == null)
		{
			instance = new TraderSpawnSavedData();
			world.getServer().getWorld(DimensionType.OVERWORLD).getSavedData().set(instance);
		}
		instance.markDirty();
		return instance;
	}
